<template>
  <div
    id="account-change-password-modal"
    tabindex="-1"
    aria-hidden="true"
      ...
      <div class="relative rounded-lg bg-white shadow dark:bg-gray-700">
        ...
        >
          ...
          <button
            ...
            data-modal-hide="account-change-password-modal"
            v-on:click="resetAll"
          >
            <svg
              class="h-3 w-3"
        ...
        <div class="container space-y-4 p-4 md:p-5">
          <div class="text-justify text-sm">
            After updating your password, you'll be logged out and need to sign
            in again with your new credentials.
          </div>
          <form
            v-on:submit.prevent="accountChangePasswordSubmit"
            class="space-y-4"
          >
            <div>
              <label
...
const passwordChange1 = ref("");
const passwordChange2 = ref("");
...
const buttonClassLoading = ref(
  "w-full text-white bg-yellow-400 dark:bg-yellow-500 cursor-not-allowed font-medium rounded-lg text-sm px-5 py-2.5 text-center",
);
...
const checkFormValidity = function () {
  if (
    ...
    !passwordChange2.value ||
    passwordChange1.value !== passwordChange2.value
  ) {
    userStore.triggerFlash(
      "warning",
      "Invalid data provided",
      "The password update form is not properly filled.",
    );
